...
// Extending DatabaseStorage with interaction methods
DatabaseStorage.prototype.toggleLike = async function(userId: string, postId: string): Promise<{ liked: boolean; likesCount: number }> {
...
DatabaseStorage.prototype.toggleFavorite = async function(userId: string, postId: string): Promise<{ favorited: boolean }> {
...
DatabaseStorage.prototype.addComment = async function(userId: string, postId: string, content: string): Promise<Comment> {
  const [newComment] = await db
    .insert(comments)
    .values({
      userId,
...
DatabaseStorage.prototype.getComments = async function(postId: string): Promise<any[]> {
  return await db
    .select({
      id: comments.id,
      content: comments.content,
...
DatabaseStorage.prototype.getFavorites = async function(userId: string): Promise<any[]> {
  return await db
    .select({
      id: posts.id,
      title: posts.title,
    ...
    .from(posts)
    .innerJoin(favorites, eq(posts.id, favorites.postId))
